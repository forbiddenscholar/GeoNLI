...
class HiViT(models.HiViT):
    """
    Detection backbone wrapper around the base HiViT model.

    Note: We rely on the full detector checkpoint (e.g. epoch_36.pth)
    to load weights, so `init_weights` is a no-op here.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # remove classification-specific layers
        ...
        if with_fpn:
            self.fpn1 = (
                ...
                if "H" not in out_indices
                else LayerNorm2d(embed_dim // 4, eps=1e-6)
            )
            self.fpn2 = (
                nn.Sequential(
                    nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=2, stride=2),
                )
                if "M" not in out_indices
                else LayerNorm2d(embed_dim // 2, eps=1e-6)
            )
    ...
    def init_weights(self):
        """
        For inference we rely on the detector checkpoint (epoch_36.pth),
        which will load backbone weights via mmdet's checkpoint loader.
        """
        return
    ...
    def interpolate_pos_encoding(self, x, h, w):
        ...
        # add a small number to avoid floating point error in interpolation
        w0, h0 = w0 + 0.1, h0 + 0.1
    ...
    def prepare_tokens(self, x, mask=None):
        ...
        for i, blk in enumerate(self.blocks[:-self.num_main_blocks]):
            ...
            if i == self.merge_indices[0] and "H" in self.out_indices:
                xp = (
                    x.reshape(B, Hp, Wp, 4, 4, -1)
                    .permute(0, 5, 1, 3, 2, 4)
                    .reshape(B, -1, Hp * 4, Wp * 4)
                    .contiguous()
                )
            ...
            if i == self.merge_indices[1] and "M" in self.out_indices:
                xp = (
                    x.reshape(B, Hp, Wp, 2, 2, -1)
                    .permute(0, 5, 1, 3, 2, 4)
                    .reshape(B, -1, Hp * 2, Wp * 2)
                    .contiguous()
                )
